...
Pass --only-orientation to rerun against just a subset of a folder you've
already processed once -- e.g. after adding portrait-mode support, you can
point at the SAME 1971-file input_folder again with a NEW, separate
work_folder and --only-orientation portrait, and it'll skip every already-
correctly-handled 6000x4000 landscape file, submitting only the 4000x6000
portrait ones. Point step2 at that new work_folder afterward; it appends
to the same output_folder's CSVs rather than overwriting them.
...
from datestamp_common_landscape_and_portrait_incl_nodate import (
    get_all_corner_crops,
    crop_to_base64_jpeg,
...
def main():
    only_orientation = None
    if "--only-orientation" in sys.argv:
        idx = sys.argv.index("--only-orientation")
        try:
            only_orientation = sys.argv[idx + 1]
        except IndexError:
            only_orientation = None
        if only_orientation not in ("portrait", "landscape"):
            print("Error: --only-orientation must be followed by 'portrait' or 'landscape'")
            sys.exit(1)

    # Positional args = everything except "--only-orientation" and the value
    # right after it (e.g. "portrait"), so that value doesn't get mistaken
    # for a third positional argument.
    skip_next = False
    args = []
    for a in sys.argv[1:]:
        if skip_next:
    ...
    if len(args) != 2:
        print("Usage: python3 step1_submit_batch.py <input_folder> <work_folder> "
              "[--only-orientation portrait|landscape]")
        sys.exit(1)

    input_folder = Path(args[0]).resolve()
    work_folder = Path(args[1]).resolve()
    ...
    if only_orientation:
        ...
        for p in images:
            try:
                orientation = get_image_orientation(p)
            except Exception as e:
                print(f"  {p.name}: couldn't read orientation ({e}) -- included to be safe", flush=True)
                kept.append(p)
                continue
            if orientation == only_orientation:
                kept.append(p)
            else:
                skipped += 1
        print(f"--only-orientation {only_orientation}: keeping {len(kept)}, "
              f"skipping {skipped} already-handled {'landscape' if only_orientation == 'portrait' else 'portrait'} file(s)",
              flush=True)
        images = kept
