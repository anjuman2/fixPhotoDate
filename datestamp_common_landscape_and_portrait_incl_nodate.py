#!/usr/bin/env python3
"""
datestamp_common_landscape_and_portrait_incl_nodate.py
...
CLAUDE_PROMPT = (
    ...
    "left-to-right, right-side up. AT MOST one of these four corners MAY "
    "contain a small orange or red digital date stamp printed by the "
    "camera -- but many photos in this collection have NO date stamp at "
    "all (the camera's date-imprint feature was off for that shot), and "
    "that is the normal, expected case for a real fraction of these "
    "images, not a failure. Do not assume a stamp must be present "
    "somewhere just because you're being asked to look for one. The other "
    "corners (or all four, if there's no stamp at all) will just show "
    "background (wall, furniture, wood grain, sky, etc.) -- ignore any "
    "warm/orange-toned wood, skin tone, or similar background color, and "
    "also ignore scan grain, JPEG compression blockiness, dust, or blurry "
    "smudges that only vaguely resemble digit shapes. Only report a stamp "
    "if you can clearly make out small, sharp, blocky LED/LCD-style digit "
    "segments forming an actual, coherent date -- not a rough pattern "
    "that merely reminds you of one. If you have real doubt about whether "
    "what you're seeing is a genuine stamp versus noise or texture, "
    "answer UNKNOWN -- an incorrect but confident-looking guess is worse "
    "than an honest UNKNOWN, since this pipeline flags UNKNOWN for a "
    "quick manual look but silently accepts whatever digits you report.\n\n"
